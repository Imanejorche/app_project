...
@section('content')
   ...
   <div class="flex flex-col w-full h-screen px-4 py-8 mt-10">
    <div class="container">
        <a href="{{ route('empCR') }}" class="btn-create-client">Create Employee</a>
    ...
    <table class="table">
        ...
        <tbody>
            @foreach ($emps as $emp)
                    ...
                    @php
                        $adresse = App\Models\Addresseemp::where('emp_id', $emp->numeroemp)->first();
                    @endphp
                    @php
                        $in = App\Models\Informationemp::where('emp_id', $emp->numeroemp)->first();
                    @endphp
                <tr>
                    ...
                    <td style="display: flex; align-items: center; margin-left: 60px;">
                        <form action="{{ route('emp.destroy', $emp->id) }}" method="POST" style="margin-right: 10px;">
                            @csrf
                            @method('DELETE')
                        ...
                        <a href="{{ route('showemp',$emp->id) }}"><img src="{{ asset('images/crayon.png') }}" alt="update" style="width: 18px; margin-right:30px"></a>
                    </td>
                </tr>
            @endforeach
