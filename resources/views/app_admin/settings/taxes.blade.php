...
@section('content')
   ...
   <div class="flex flex-col w-full h-screen px-4 py-8 mt-10">
    <div class="container">
        <a href="{{ route('crtaxe') }}" class="btn-create-client"> Create Tax</a>
    </div>
        ...
        <tbody>
            @foreach ($taxes as $taxe)
                <tr>
                    <td>{{ $taxe->nom }}</td>
                    <td>{{ $taxe->tr }}</td>
                    <td>{{ $taxe->taux_de_taxe }}</td>

                    <td style="display: flex; align-items: center; margin-left: -30px;">
                        <form action="{{ route('taxe.destroy', $taxe->id) }}" method="POST" style="margin-right: 10px;">
                            @csrf
                            @method('DELETE')
                        ...
                        <a href="{{ route('showtaxe', $taxe->id) }}"><img src="{{ asset('images/update2.png') }}" alt="update" style="width: 21px; margin-right:20px"></a>
                    </td>
                </tr>
            @endforeach
        </tbody>
