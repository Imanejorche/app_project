...
@section('content')
   ...
   <div class="flex flex-col w-full h-screen px-4 py-8 mt-10">
    <div class="container">
        <a href="{{ route('createprix') }}" class="btn-create-client">Create Price level</a>
    </div>
            ...
            @foreach ($prixs as $prix)
                <tr>
                    ...
                    <td style="display: flex; align-items: center; margin-left: -30px;">
                        <form action="{{ route('prix.destroy', $prix->id) }}" method="POST" style="margin-right: 10px;">
                            @csrf
                            @method('DELETE')
                        ...
                        <a href="{{ route('showprix', $prix->id) }}"><img src="{{ asset('images/update2.png') }}" alt="update" style="width: 21px; margin-right:20px"></a>
                    </td>
                </tr>
            @endforeach
        </tbody>
